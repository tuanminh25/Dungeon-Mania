...
public class Game {
    ...
    private void updateControlledEnemies() {
        List<Mercenary> enemies = map.getEntities(Mercenary.class);
        enemies.forEach(e -> {
            registerOnce(() -> e.controlUpdate(), Game.PLAYER_MOVEMENT_CALLBACK, e.getId());
        });
    }
    ...
    public List<Exit> getExits() {
        return map.getEntities(Exit.class);
    }

    public List<Switch> getSwitches() {
        return map.getEntities(Switch.class);
    }
    ...
    public void moveTo(Entity entity, Position pos) {
        map.moveTo(entity, pos);
    }
    ...
    public int getPlayerEnemyCount() {
        return player.getEnemyCount();
    }

    public boolean isSpawnerActive() {
        return map.countSpawner() == 0;
    }

    public boolean isZombieActive() {
        return map.countZombie() > 0;
    }
}
