...
import dungeonmania.entities.logical.rule.RuleFactory;
...
public class EntityFactory {
    ...
    public Sceptre buildSceptre() {
        // the item can only be used one
        int sceptreDurability = 1;
        int sceptreControlTime = config.optInt("mind_control_duration");
        return new Sceptre(sceptreDurability, sceptreControlTime);
    }

    public Buildable buildMidNightAmour() {
        int durability = 2000000000;
        double shieldDefence = config.optInt("midnight_armour_defence");
        double shieldAttack = config.optInt("midnight_armour_attack");
        return new MidnightArmour(durability, shieldDefence, shieldAttack);
    }
    ...
    private Entity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "bomb":
            ...
            if (jsonEntity.has("logic")) {
                rule = factory.createRule(jsonEntity.getString("logic"));
                return new LogicalBomb(pos, bombRadius, rule);
            } else {
        ...
        case "light_bulb_off":
            rule = factory.createRule(jsonEntity.getString("logic"));
            return new LightBulb(pos, rule);
        case "switch_door":
            rule = factory.createRule(jsonEntity.getString("logic"));
            return new SwitchDoor(pos, rule);
        case "sun_stone":
